...
export default function Effects() {
  ...
  useEffect(() => {
    ...
    let gradedRgb = finalNode.rgb.mul(float(MEADOW_POST_BLACKS));
    const gradeLuma = luminance(gradedRgb);
    const shadowMask = smoothstep(float(MEADOW_POST_SHADOW_LUMA_MAX), float(0), gradeLuma);
    gradedRgb = mix(gradedRgb, gradedRgb.mul(float(MEADOW_POST_SHADOWS)), shadowMask);
